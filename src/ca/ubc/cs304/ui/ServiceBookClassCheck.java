...
public class ServiceBookClassCheck extends JFrame {
    private TerminalTransactionsDelegate delegate;
    public ServiceBookClassCheck(ServiceBookClass optionWindow, TerminalTransactionsDelegate delegate) {
        this.delegate = delegate;

        BookingModel[] model = delegate.getAvailableSlots();
        Object[][] data = new Object[model.length][4];
        for (int i = 0; i < model.length; i++) {
            data[i][0] = model[i].getIID();
            data[i][1] = model[i].getClassName();
            data[i][2] = model[i].getClassDate();
            data[i][3] = model[i].getNumberOfAvailableSpots();
        }
        ...
        // Create column names
        String[] columnNames = {"Instructor ID", "Class Name", "Class Date", "Number of Available Spots"};
